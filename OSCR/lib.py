        ...
        if ix not in index:
            if ix == 0 or ix == 1:
                try:
                    ...
                    raise TypeError(
                        f"The first table column must contain a str, not {type(item).__name__}"
                    )
            else:
                yield item
...
def analysis_parser(path, pipe, id=None):
    ...
    if id is None:
        ...
        uiD, dmgI, healI, uiID, _, _, _, _, _, npcdmg, npcdps = parser.readCombatFull(
            path
        )
        ...
        (
            ...
            npcdmg,
            npcdps,
        ) = parser.readPreviousCombatFull(id)
    pipe.send(uiD)
...
class StandardItem(QStandardItem):
    ...
    def __init__(self, value):
        if not isinstance(value, (str, int, float)):
            raise TypeError(
                f"StandardItem only supports str, int or float; not {type(value).__name__}"
            )
        self._val = value
        super().__init__()
    ...
    def set_val(self, val):
        if not isinstance(val, (str, int, float)):
            raise TypeError(
                f"StandardItem only supports str, int or float; not {type(val).__name__}"
            )
        self._val = val
