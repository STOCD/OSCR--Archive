...
from src.lib import (CustomThread, StandardItem, analysis_parser,
                     clean_player_id, filtered_ability, get_entity_num,
                     resize_tree_table, std_item_generator)
from src.ui.widgets import ACENTER, ARIGHT, AVCENTER
...
class DataWrapper:
    ...
    def analyze_log_callback(self, combat_id=None, path=None):
        ...
        if combat_id is None:
            if not path or not os.path.isfile(path):
                self.show_warning(
                    "Invalid Logfile",
                    "The Logfile you are trying to open does not exist.",
                )
                return
            ...
            analysis_thread = CustomThread(
                self.window, lambda: self.get_analysis_data(path)
            )
        ...
        # subsequent run / click on older combat
            ...
            analysis_thread = CustomThread(
                self.window,
                lambda: self.get_analysis_data(
                    self.current_combat_path, reversed_combat
                ),
            )
            analysis_thread.start()
    ...
    def get_data(self, combat=None, path=None) -> Union[list, None]:
        ...
        if combat is None:
            ...
            (
                ...
                _,
                _,
            ) = self.parser.readCombatShallow(path)
        ...
        # same log file, old combat
            ...
            _, _, _, dmg_data, dps_data, _, _ = self.parser.readPreviousCombatShallow(
                combat
            )
            front_page_data = self.parser.createFrontPageTable()
    ...
    def populate_analysis(self):
        ...
        model = TreeModel(
            DAMAGE_HEADER,
            self.theme_font("tree_table_header"),
...
class TreeModel(QStandardItemModel):
    ...
    def populate_in(self, data, index):
        ...
        """
        player, player_row = std_item_generator(
            ["Player"] + [""] * (len(self._header) - 1), ()
        )
        npc, npc_row = std_item_generator(["NPC"] + [""] * (len(self._header) - 1), ())
        for player_name, stats in data.items():
            if self._is_passive(stats):
                continue
            current_entity, current_row = std_item_generator(
                [player_name] + [""] * (len(self._header)), (1,)
            )
    ...
    def populate_dout(self, data, index):
        ...
        for stats in data.values():
            ...
            try:
                ...
                current_entity, current_row = std_item_generator(
                    [stats[1][0][0]] + [""] * (len(self._header)), (1,)
                )
            pet_sum_set = False
            pet_sum = None
            if isinstance(stats[index], list):
    ...
    def insert_ability(
        ...
        """
        global_, global_row = std_item_generator(
            ability[0], (name_index,)
        )  # self._to_standard_item(filtered_ability(ability[0], (1,)))
                ...
                if len(ability) == 2:
                    ...
                    if isinstance(line[1][0], list):
                        for child_line in line[1:]:
                            self.insert_ability(
                                child_line, global_, name_index, sub_index
                            )
                    else:
                        for child_line in line[1:]:
                            global_.appendRow(
                                std_item_generator(child_line, (sub_index,))[1]
                            )
                else:
