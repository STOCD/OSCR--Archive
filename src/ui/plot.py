...
from src.data import SortingProxy, TableModel
from src.lib import CustomThread, clean_player_id
from src.ui.widgets import RFIXED, SMINMIN, SMPIXEL


class PlotWrapper:
    from src.ui.styles import get_style_class, theme_font
    ...
    def create_overview_dmg(self) -> Figure:
        ...
        with plt.style.context(self.config["plot_stylesheet_path"]):
            ...
            a.legend(
                loc="upper center", bbox_to_anchor=(0.5, -0.025), ncol=3, frameon=False
            )
    ...
    def create_overview_dps(self):
        ...
        with plt.style.context(self.config["plot_stylesheet_path"]):
            ...
            for player, array in self.overview_data[3].items():
                a.plot(
                    np.divide(array[0], 1000), array[1], label=clean_player_id(player)
                )
            a.legend(
                loc="upper center", bbox_to_anchor=(0.5, -0.025), ncol=3, frameon=False
            )
    ...
    def create_bar_labels(self, axis: Axes, bars, y, names, x):
        ...
        axis.bar_label(
            bars,
            labels=labels,
            color=self.theme["defaults"]["fg"],
            fontsize=17,
            padding=self.theme["defaults"]["margin"],
            label_type="edge",
            fontweight="bold",
        )
    ...
    def create_overview_table(self) -> QTableView:
        ...
        """
        model = TableModel(
            *self.overview_data[0:3],
            header_font=self.theme_font("table_header"),
            cell_font=self.theme_font("table"),
        )
        sort = SortingProxy()
        ...
        table.horizontalHeader().setStyleSheet(
            self.get_style_class("QHeaderView", "table_header")
        )
        table.verticalHeader().setStyleSheet(
            self.get_style_class("QHeaderView", "table_index")
        )
        table.resizeColumnsToContents()
        for col in range(len(model._header)):
            table.horizontalHeader().resizeSection(
                col, table.horizontalHeader().sectionSize(col) + 5
            )
