...
from PyQt6.QtWidgets import (QApplication, QFrame, QGridLayout, QHBoxLayout,
                             QLineEdit, QListWidget, QTabWidget, QVBoxLayout,
                             QWidget)
from src.data import DAMAGE_HEADER, HEAL_HEADER, DataWrapper
from src.lib import set_variable
from src.ui.plot import PlotWrapper
from src.ui.widgets import (ACENTER, ALEFT, ARIGHT, ATOP, SMAXMAX, SMAXMIN,
                            SMINMAX, SMINMIN, BannerLabel, FlipButton,
                            WidgetBuilder)


class OscrGui(WidgetBuilder, DataWrapper, PlotWrapper):
    from src.io import browse_path, get_asset_path
    from src.ui.styles import create_style_sheet, get_style_class
    ...
    def setup_main_layout(self):
        ...
        right = self.create_frame(
            main_frame,
            "frame",
            {
                "border-left-style": "solid",
                "border-left-width": "@sep",
        ...
        left_flip_config = {
            "icon_r": self.icons["collapse-left"],
            "func_r": left.hide,
            "icon_l": self.icons["expand-left"],
            "func_l": left.show,
        }
        right_flip_config = {
            "icon_r": self.icons["expand-right"],
            "func_r": right.show,
            "icon_l": self.icons["collapse-right"],
            "func_l": right.hide,
        }
        for col, config in ((1, left_flip_config), (3, right_flip_config)):
            ...
            flip_button.setStyleSheet(
                self.get_style_class("QPushButton", "small_button")
            )
    ...
    def setup_left_sidebar(self, frame: QFrame):
        ...
        entry_button_config = {
            ...
            "Browse ...": {
                "callback": lambda: self.browse_log(self.entry),
                "align": ALEFT,
            },
            "Analyze": {
        ...
        entry_buttons = self.create_button_series(
            button_frame, entry_button_config, "button"
        )
        button_frame.setLayout(entry_buttons)
        left_layout.addWidget(button_frame)

        refresh_button = self.create_icon_button(
            self.icons["refresh"], "icon_button", frame
        )
        ...
        self.current_combats.setStyleSheet(
            self.get_style_class("QListWidget", "listbox")
        )
    ...
    def setup_main_tabber(self, frame: QFrame):
        ...
        main_tabber.tabBar().setStyleSheet(
            self.get_style_class("QTabBar", "tabber_tab")
        )
        ...
        self.widgets["main_menu_buttons"][0].clicked.connect(
            lambda: main_tabber.setCurrentIndex(0)
        )
        self.widgets["main_menu_buttons"][1].clicked.connect(
            lambda: main_tabber.setCurrentIndex(1)
        )
        self.widgets["main_menu_buttons"][2].clicked.connect(
            lambda: main_tabber.setCurrentIndex(2)
        )
        self.widgets["main_menu_buttons"][3].clicked.connect(
            lambda: main_tabber.setCurrentIndex(3)
        )
    ...
    def setup_overview_frame(self):
        ...
        switch_style = {
            ...
            "DPS Bar": {
                "callback": lambda: o_tabber.setCurrentIndex(0),
                "align": ACENTER,
            },
            "DPS Graph": {
                "callback": lambda: o_tabber.setCurrentIndex(1),
                "align": ACENTER,
            },
            "Damage Graph": {
                "callback": lambda: o_tabber.setCurrentIndex(2),
                "align": ACENTER,
            },
        }
        switcher, buttons = self.create_button_series(
            switch_frame, switch_style, "button", ret=True
        )
    ...
    def setup_analysis_frame(self):
        ...
        self.widgets["analysis_tab_frames"].extend(
            (dout_frame, dtaken_frame, hout_frame, hin_frame)
        )
        layout = QVBoxLayout()
        layout.setContentsMargins(0, 0, 0, 0)
        layout.setSpacing(0)
        ...
        switch_style = {
            "default": {"margin-left": "@margin", "margin-right": "@margin"},
            "Damage Out": {
                "callback": lambda: a_tabber.setCurrentIndex(0),
                "align": ACENTER,
            },
            "Damage Taken": {
                "callback": lambda: a_tabber.setCurrentIndex(1),
                "align": ACENTER,
            },
            "Heals Out": {
                "callback": lambda: a_tabber.setCurrentIndex(2),
                "align": ACENTER,
            },
            "Heals In": {
                "callback": lambda: a_tabber.setCurrentIndex(3),
                "align": ACENTER,
            },
        }
        switcher, buttons = self.create_button_series(
    ...
    def create_master_layout(self, parent) -> tuple[QVBoxLayout, QFrame]:
        ...
        lbl = BannerLabel(
            self.get_asset_path("oscrbanner-slim-dark-label.png"), bg_frame
        )
    ...
    def setup_settings_frame(self):
        ...
        for i, head in enumerate(DAMAGE_HEADER[1:]):
            ...
            bt.clicked.connect(
                lambda state, i=i: set_variable(self.settings["dmg_columns"], i, state)
            )
        ...
        apply_button = self.create_button(
            "Apply", "button", col_1_frame, {"margin-top": 15}
        )
        ...
        heal_hider_label = self.create_label(
            "Heal table columns:", "label", col_2_frame
        )
        ...
        for i, head in enumerate(HEAL_HEADER[1:]):
            ...
            bt.clicked.connect(
                lambda state, i=i: set_variable(self.settings["heal_columns"], i, state)
            )
        ...
        apply_button_2 = self.create_button(
            "Apply", "button", col_2_frame, {"margin-top": 15}
        )
